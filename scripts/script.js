  ...
  var sortTableRows = function(table, order) {
    ...
    while (switching) {
      ...
      for (i = 1; i < rows.length - 2; i++) {
        ...
        if (order === "low-high") {
          if (
            parseInt(x[1].innerHTML.substring(1)) >
            parseInt(y[1].innerHTML.substring(1))
          ) {
            Switch = true;
            break;
          }
  ...
  return {
    getIncomeInput: function() {
      return {
        ...
        amount: parseFloat(
          document.querySelector(DOMStrings.incomeAmount).value
        )
      };
    },
    getExpenseInput: function() {
      return {
        ...
        amount: parseFloat(
          document.querySelector(DOMStrings.expenseAmount).value
        )
      };
    },
    ...
    addListItem: function(obj, type) {
      ...
      if (type === "+") {
        ...
        html =
          '<tr id="expense-%id%"><td class="expense_name_row table-heading">%name%</td><td class="expense_amount_row table_float_right ">£%amount%</td><td class="expense_description_row table-heading">%description%</td><td class="delete_edit_expense table-heading"><a class="edit"  data-toggle="modal" data-target="#editExpenseModal" href="#editExpenseModal" style="text-decoration: underline; color: black;">Edit</a> | <a class="delete" style="text-decoration: underline">Delete</a></td></tr>';
      }
    ...
    clearFields: function(type) {
      ...
      if (type === "+") {
        fields = document.querySelectorAll(
          DOMStrings.incomeName +
            ", " +
            DOMStrings.incomeDesc +
            ", " +
            DOMStrings.incomeAmount
        );
        var modal = document.getElementById("incomeModal");
      } else if (type === "-") {
        fields = document.querySelectorAll(
          DOMStrings.expenseName +
            ", " +
            DOMStrings.expenseDesc +
            ", " +
            DOMStrings.expenseAmount
        );
    ...
    checkEmptyFields: function(type) {
      ...
      if (type === "+") {
        fields = document.querySelectorAll(
          DOMStrings.incomeName + ", " + DOMStrings.incomeAmount
        );
      } else if (type === "-") {
        fields = document.querySelectorAll(
          DOMStrings.expenseName + ", " + DOMStrings.expenseAmount
        );
      }
    ...
    displayBudget: function(obj) {
      ...
      document.querySelector(DOMStrings.monthlyTotal).textContent =
        "£" + formatNumber(obj.budget, type);
      document.querySelector(DOMStrings.totalIncome).textContent =
        "£" + formatNumber(obj.totalInc, "income");
      document.querySelector(DOMStrings.totalExpense).textContent =
        "£" + formatNumber(obj.totalExp, "expense");
      document.querySelector("#income-table-total").textContent =
        "£" + formatNumber(obj.totalInc, "income");
      document.querySelector("#expense-table-total").textContent =
        "£" + formatNumber(obj.totalExp, "expense");
    },
...
var controller = (function(dataCtrl, UICtrl) {
  ...
  var setUpEventListeners = function() {
    document
      .querySelector(DOM.incomeButton)
    ...
    document
      .querySelector(DOM.expenseButton)
    ...
    document
      .querySelector(".main-row")
      .addEventListener("click", ctrlDeleteItem);

    document.querySelector(".incomes").addEventListener(
      "input",
      function(event) {
    ...
    document.querySelector(".expenses").addEventListener(
      "input",
      function(event) {
